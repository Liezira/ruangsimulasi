...
import { auth } from './firebase'; // Pastikan path ini benar sesuai struktur foldermu
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isRegister) {
        ...
        // Update Nama Display (Penting agar tidak null)
        await updateProfile(userCredential.user, {
          displayName: name,
        });

        // Note: Penyimpanan ke Firestore ditangani oleh useEffect di App.jsx (Centralized)
      } else {
        // 2. LOGIN FLOW
      ...
      // Translate Error Firebase ke Bahasa Manusia
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              {isRegister ? 'Bergabung Sekarang' : 'Selamat Datang Kembali'}
            </h2>
            <p className="text-gray-500 text-sm">
              {isRegister
                ? 'Buat akun untuk memulai simulasi UTBK.'
                : 'Masuk untuk mengakses dashboard ujianmu.'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition outline-none"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 mt-4"
            >
              {isLoading ? (
